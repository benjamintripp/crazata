...
world.beforeEvents.worldInitialize.subscribe((initEvent) => {
    ...
    initEvent.itemComponentRegistry.registerCustomComponent("nate:teleporter", {
        onUse(arg) {
            const player = arg.source;
            const blockRequest = player.getBlockFromViewDirection({ maxDistance: 100 });

            if (blockRequest) {
                const block = blockRequest.block;
                const location = block.location; // Extract block location
    
                // Teleport the player to the block's location
                player.tryTeleport({ x: location.x, y: location.y + 1, z: location.z });
            }
        },

        onHitEntity(arg) {
            const hitEntity = arg.hitEntity;
            const currentDimension = hitEntity.dimension.id;
            const currLocation =  hitEntity.location;


            // Generate random offsets for X and Z (between -10 and +10)
            const randomOffsetX = Math.random() * (10 - 5) + 5; // Random between 5 and 10
            const randomOffsetZ = Math.random() * (10 - 5) + 5;

            // Randomly flip directions (negative or positive)
            const offsetX = Math.random() < 0.5 ? randomOffsetX : -randomOffsetX;
            const offsetZ = Math.random() < 0.5 ? randomOffsetZ : -randomOffsetZ;

            // Calculate the new location
            const newLocation = {
                x: currLocation.x + offsetX,
                y: currLocation.y, // No vertical change
                z: currLocation.z + offsetZ,
            };

            // Teleport the hit entity to the new location
            hitEntity.teleport(newLocation);
        }
    });

    initEvent.itemComponentRegistry.registerCustomComponent("nate:burn", {
        onHitEntity(arg) {
